...
CREATE_INSTANCE() {
  ...
  sed -e "s/IPADDRESS/${IPADDRESS}/" -e "s/COMPONENT/${COMPONENT}/" myrecord.json >/tmp/myrecord.json
  aws route53 change-resource-record-sets --hosted-zone-id ${ZONE_ID} --change-batch file:///tmp/myrecord.json | jq
}
